...
class ATSBaseError(Exception):
    ...
    def __init__(self, message: str, user_message: Optional[str] = None, **kwargs):
        ...
        self.user_message = user_message or 'Any error occurred. Please check the logs for more details.'
...
def log_error(error: Exception, context: Optional[str] = None, **kwargs) -> None:
    """Log an error simply."""
    logger.info(f"Error in {context or 'unknown'}: {error}")

def log_warning(error: Exception, context: Optional[str] = None, **kwargs) -> None:
    """Log a warning simply."""
    logger.warning(f"{context}: {message}" if context else message)

def log_info(error: Exception, context: Optional[str] = None, **kwargs) -> None:
    """Log an info simply."""
